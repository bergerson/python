...
z, w = [], []
for line in open('beta_ceph.txt', 'r'):
  values = [float(s) for s in line.split()]
  z.append(values[0])
  w.append(values[1])

line1, = plt.plot(z, w, linestyle= '-', linewidth=1, markersize=0.1,  color='gold', label= r'$\mathrm{\beta \ Cep. \ Area}$')

#legend = ax.legend(loc='upper center', shadow=True, fontsize='x-large')

a, b = [], []
for line in open('spb.txt', 'r'):
  values = [float(s) for s in line.split()]
  a.append(values[0])
  b.append(values[1])

line2, = plt.plot(a, b, linestyle= '-', markersize=2, color='red', label= r'$\mathrm{SPB \ Area}$')

# Create another legend for the first and second line.
